...
Response HttpServerBase::process_routes(const Request& request) {
    // Find matching route
    for (auto& route : routes_) {
        if (route->matches(request.path(), request.method())) {
            ...
            return apply_middleware(modified_request, [&route](const Request& req) {
...
Response HttpServerBase::apply_middleware(const Request& request,
                                         std::function<Response(const Request&)> handler) {
    // Apply middleware in order
    std::function<Response(const Request&)> chain = handler;
    
    for (auto it = middleware_.rbegin(); it != middleware_.rend(); ++it) {
        auto middleware = *it;
        auto next_chain = chain;
        chain = [middleware, next_chain](const Request& req) {
