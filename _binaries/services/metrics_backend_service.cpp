...
private:
    ...
    std::string generate_api_docs() {
        ml::networking::html::ApiDocGenerator docs("Metrics Backend API", "1.0");
        docs.set_description("System monitoring and metrics collection API")
            .set_base_url("http://localhost:" + std::to_string(port_));

        docs.add_endpoint("GET", "/metrics", "Get current system metrics including CPU, memory, disk, and network usage")
            .with_response_body(R"({
  "cpu": 45.2,
  "memory": 62.1,
  "disk": 78.5,
  "network_rx": 12.3,
  ...
  "timestamp": 1609459200
            ...
            .with_response_code("200 OK");

        docs.add_endpoint("GET", "/api/metrics", "Alternative endpoint for system metrics (same as /metrics)")
            .with_response_body(R"({
  "cpu": 45.2,
  "memory": 62.1,
  "disk": 78.5,
  ...
  "timestamp": 1609459200
        ...
        docs.add_endpoint("GET", "/health", "Health check endpoint to verify service is running")
            .with_response_body(R"({
  "status": "healthy",
  "service": "metrics-backend"
        ...
        docs.add_endpoint("GET", "/api/docs", "This API documentation page")
            .with_response_code("200 OK");

        return docs.generate_html();
    }
    ...
    void handle_request(int client_socket) {
        ...
        // Handle GET /api/docs
        else if (request.find("GET /api/docs") != std::string::npos) {
            std::string html = generate_api_docs();
            std::string response = http_response(html, "text/html");
            send(client_socket, response.c_str(), response.length(), 0);
        }
        // Handle GET /health
...
public:
    ...
    bool start() {
        ...
        std::cout << "  - GET /api/docs      - API documentation\n";
...
int main(int argc, char* argv[]) {
    // Disable buffering for immediate output capture
    setbuf(stdout, NULL);
    setbuf(stderr, NULL);
    std::cout.setf(std::ios::unitbuf);
    std::cerr.setf(std::ios::unitbuf);
    
    std::cout << "[Metrics Backend] Starting..." << std::endl;
