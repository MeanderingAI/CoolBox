...
private:
    ...
    bool reload_nginx() {
        ...
        if (reload_result == 0) {
            std::cout << "[nginx] ✓ Nginx reloaded successfully\n";
            return true;
        } else {
                ...
                if (stat(filepath.c_str(), &file_stat) == 0) {
                    ...
                    if (watched_files_mtimes_.find(filepath) != watched_files_mtimes_.end()) {
                        if (watched_files_mtimes_[filepath] != mtime) {
                            ...
                            std::cout << "\n╔════════════════════════════════════════════════════════════════╗\n";
                            std::cout << "║  🔄 SOURCE FILE CHANGED - AUTO-REBUILD TRIGGERED           ║\n";
                            std::cout << "╠════════════════════════════════════════════════════════════════╣\n";
                            std::cout << "║  Time:     " << time_buf << "." << std::setfill('0') << std::setw(3) << now_ms.count() << std::setfill(' ') << "                                           ║\n";
                            std::cout << "║  File:     " << filename << std::string(51 - filename.length(), ' ') << "║\n";
                            std::cout << "║  Path:     " << filepath.substr(base_path.length() + 1) << std::string(51 - filepath.substr(base_path.length() + 1).length(), ' ') << "║\n";
                            std::cout << "╚════════════════════════════════════════════════════════════════╝\n\n";
                            ...
                            if (service_id > 0) {
                                ...
                                std::cout << "  ℹ️  Header file changed - rebuilding all services that may depend on it...\n\n";
                                for (const auto& [svc_name, svc_id] : service_map) {
                            ...
                            for (int svc_id : services_to_rebuild) {
                                ...
                                std::cout << "  🔨 Building: " << service_name << "...\n";
                                ...
                                if (build_success) {
                                    ...
                                    if (was_running) {
                                        ...
                                        std::cout << "     ℹ️  Service was not running, not restarting\n\n";
                                    }
                                } else {
                                    std::cout << "     ✗ Build failed! Check output above\n\n";
                                }
                            }
...
public:
    ServiceManager() {
        ...
        if (use_nginx_) {
            std::cout << "✓ nginx detected - service routing via nginx enabled\n";
            ...
            if (!reload_nginx()) {
                std::cout << "⚠️  nginx reload failed - falling back to direct routing\n";
                use_nginx_ = false;
            }
        } else {
        ...
        std::cout << "✓ Service file watcher enabled (Watchdog)\n";
    ...
    bool start_service(int id) {
        ...
        if (id >= 4 && id <= 6 && svc.command.find("bun") != std::string::npos) {
            if (!is_bun_installed()) {
                ...
                svc.output_lines.push_back("\033[31m✗ Bun.js is not installed!\033[0m");
            ...
            if (result > 0) {
                ...
                if (port_conflict) {
                    ...
                    if (!existing_pid_str.empty()) {
                        ...
                        svc.output_lines.push_back("");
                        svc.output_lines.push_back("╔═══════════════════════════════════════════════════════════════════════╗");
                        svc.output_lines.push_back("║  ⚠️  PORT CONFLICT DETECTED                                           ║");
                        svc.output_lines.push_back("╚═══════════════════════════════════════════════════════════════════════╝");
                        svc.output_lines.push_back("");
                        ...
                        svc.output_lines.push_back("");
                        svc.output_lines.push_back("1️⃣  Kill the conflicting process:");
                        svc.output_lines.push_back("    kill " + existing_pid_str + "     (or use Kill button above)");
                        svc.output_lines.push_back("");
                        svc.output_lines.push_back("2️⃣  Change this service's port:");
                        svc.output_lines.push_back("    • Edit the port field in the service table above");
                        svc.output_lines.push_back("    • Suggested alternative: " + std::to_string(svc.port + 10));
                        svc.output_lines.push_back("");
                        svc.output_lines.push_back("3️⃣  Auto-assign a free port:");
                        svc.output_lines.push_back("    • Click 'Auto-Port' button (if available)");
                        svc.output_lines.push_back("");
                        svc.output_lines.push_back("After resolving, click START to retry.");
                        svc.output_lines.push_back("");
                        
                        std::cout << "\n╔═══════════════════════════════════════════════════════════════════════╗\n";
                        std::cout << "║  ⚠️  PORT CONFLICT: Service " << id << " (" << svc.name << ")                  ║\n";
                        std::cout << "╠═══════════════════════════════════════════════════════════════════════╣\n";
                        std::cout << "║  Port " << port_str << " is in use by:                                            ║\n";
                        std::cout << "║    PID: " << existing_pid_str << std::string(58 - existing_pid_str.length(), ' ') << "║\n";
                        std::cout << "║    Process: " << process_name << std::string(53 - process_name.length(), ' ') << "║\n";
                        std::cout << "╠═══════════════════════════════════════════════════════════════════════╣\n";
                        std::cout << "║  Solutions:                                                           ║\n";
                        std::cout << "║    1. kill " << existing_pid_str << "    (terminate the process)                        ║\n";
                        std::cout << "║    2. Change port to " << (svc.port + 10) << "   (edit in UI)                          ║\n";
                        std::cout << "╚═══════════════════════════════════════════════════════════════════════╝\n\n";
                    }
                }
    ...
    bool auto_assign_port(int id) {
        ...
        if (free_port > 0) {
            ...
            svc.output_lines.push_back("");
            svc.output_lines.push_back("✓ Auto-assigned free port: " + std::to_string(free_port));
...
public:
    ...
    void start() {
        ...
        if (bind(server_fd_, (struct sockaddr*)&address, sizeof(address)) < 0) {
            std::cerr << "✗ Failed to bind to port " << port_ << " (already in use?)\n";
            close(server_fd_);
            return;
        }
        ...
        std::cout << "✓ Request logging enabled\n";
        std::cout << "✓ Thread-per-request mode\n" << std::flush;
...
<body>
    ...
    std::string load_html_with_reload(const std::string& file_path) {
        ...
        std::cout << "\n";
        std::cout << "╔════════════════════════════════════════════════════════════════╗\n";
        std::cout << "║  🔄 \033[1m\033[33mFILE CHANGE DETECTED\033[0m                                    ║\n";
        std::cout << "╠════════════════════════════════════════════════════════════════╣\n";
        std::cout << "║  Time:     " << time_buf << "." << std::setfill('0') << std::setw(3) << now_ms.count() << "                                           ║\n";
        std::cout << "║  File:     " << std::left << std::setw(50) << file_path.substr(file_path.find_last_of("/") + 1) << "║\n";
        std::cout << "║  Path:     " << std::left << std::setw(50) << file_path << "║\n";
        std::cout << "║  Status:   \033[32m✓ Reloaded\033[0m                                          ║\n";
        std::cout << "╚════════════════════════════════════════════════════════════════╝\n";
        std::cout << "\n";
...
function setPort(serviceName) {
    ...
    std::string create_frontend_boilerplate(const std::string& post_data) {
        ...
        server_file << "  }\n";
        server_file << "});\n\n";
        server_file << "console.log(`🚀 " << name << " running on http://localhost:" << port << "`);\n";
        server_file.close();
...
void print_service_tui(ServiceManager& manager, RequestLogger* logger = nullptr) {
    ...
    std::cout << "\033[1m⚙️  BACKEND SERVICES\033[0m\n";
    std::cout << "┌────┬─────────────────────────┬──────┬──────────┬───────────────────────┐\n";
    ...
    std::cout << "\033[1m🎨 FRONTEND SERVICES\033[0m\n";
    std::cout << "┌────┬─────────────────────────┬──────┬──────────┬───────────────────────┐\n";
    std::cout << "│ ID │ SERVICE NAME            │ PORT │ STATUS   │ DESCRIPTION           │\n";
    std::cout << "├────┼─────────────────────────┼──────┼──────────┼───────────────────────┤\n";
    ...
    // Request Log Section
    if (logger) {
        std::cout << "\033[1m📊 RECENT HTTP REQUESTS (Last 15)\033[0m\n";
        std::cout << "┌─────────────┬────────┬──────────────────────────────────────────────────┐\n";
        std::cout << "│ TIMESTAMP   │ METHOD │ PATH                                             │\n";
        std::cout << "├─────────────┼────────┼──────────────────────────────────────────────────┤\n";
        
        auto logs = logger->get_recent_logs(15);
        if (logs.empty()) {
            std::cout << "│ \033[90mNo requests yet...                                                 \033[0m│\n";
        } else {
    ...
    std::cout << "║                              COMMANDS                                 ║\n";
    std::cout << "╠═══════════════════════════════════════════════════════════════════════╣\n";
    std::cout << "║  \033[1mBackend Services:\033[0m                                                     ║\n";
    std::cout << "║  \033[1m[1-3]\033[0m      - Start backend by ID                                    ║\n";
    std::cout << "║  \033[1ms [1-3]\033[0m   - Stop backend by ID                                     ║\n";
    std::cout << "║  \033[1mr [1-3]\033[0m   - Restart backend by ID                                  ║\n";
    std::cout << "║  \033[1mlog [1-3]\033[0m - View backend logs                                       ║\n";
    std::cout << "║                                                                       ║\n";
    std::cout << "║  \033[1mFrontend Services:\033[0m                                                    ║\n";
    std::cout << "║  \033[1mf [4-6]\033[0m   - Start frontend by ID (e.g., 'f 4')                     ║\n";
    std::cout << "║  \033[1msf [4-6]\033[0m  - Stop frontend by ID (e.g., 'sf 4')                     ║\n";
    std::cout << "║  \033[1mrf [4-6]\033[0m  - Restart frontend by ID (e.g., 'rf 4')                  ║\n";
    std::cout << "║  \033[1mlog [4-6]\033[0m - View frontend logs                                      ║\n";
    std::cout << "║                                                                       ║\n";
    std::cout << "║  \033[1mGeneral:\033[0m                                                              ║\n";
    ...
    std::cout << "║  \033[1m➜\033[0m Account UI: \033[4mhttp://localhost:3000\033[0m (when service 4 running)     ║\n";
    std::cout << "╚═══════════════════════════════════════════════════════════════════════╝\n";
...
int main(int argc, char* argv[]) {
    ...
    if (!is_service_manager) {
        std::cout << "\n╔═══════════════════════════════════════════════════════════════════════╗\n";
        std::cout << "║        🚀 MATLAB-Style Platform - Starting Services 🚀               ║\n";
        std::cout << "╚═══════════════════════════════════════════════════════════════════════╝\n\n";
    }
