#include "security/auth/auth_system.h"
#include <sstream>
#include <iomanip>
#include <iostream>
#include <random>
...
extern "C" {
    ...
    const char* get_library_name() { return "auth"; }
    
    __attribute__((visibility("default"), used))
    const char* get_library_version() { return "1.0.0"; }
    
    __attribute__((visibility("default"), used))
    const char* get_library_description() { return "Authentication and authorization system with session management, role-based access control, and secure password handling"; }
