...
public:
    ...
    // Run 'make help' in the workspace and return output lines
    static inline std::vector<std::string> make_help(const std::string& workspace_path = ".") {
        std::string cmd = "cd " + workspace_path + " && make help";
        return run(cmd);
    }

    // Returns the process name using the given port, or empty string if free
    static inline std::string get_process_on_port(int port) {
        std::string cmd = "lsof -i :" + std::to_string(port) + " -sTCP:LISTEN -n -P | awk 'NR>1 {print $1}' | head -n 1";
